...
type CommandTemplatesConfig struct {
	...
	// this to "podman compose -f foo/docker-compose.yml -f
	...
	// AllLogs is for showing what you get from doing `podman compose logs`. It
...
func NewAppConfig(name, version, commit, date string, buildSource string, debuggingFlag bool, composeFiles []string, projectDir string) (*AppConfig, error) {
	...
	// Pass compose files as individual -f flags to podman compose
